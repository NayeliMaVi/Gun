...
def build_model() -> tf.keras.Model:
    ...
    model = models.Model(inputs=inputs, outputs=outputs, name="nasnetmobile_weapon_validator")
    model.compile(
...
def build_callbacks(paths: dict[str, Path]) -> list[tf.keras.callbacks.Callback]:
    best_model_path = paths["models_dir"] / "nasnetmobile_weapon_validator_best.keras"
    return [
...
def save_test_metrics(metrics: dict[str, float], output_txt: Path, class_names: list[str]) -> None:
    with output_txt.open("w", encoding="utf-8") as handle:
        handle.write("NasNetMobile test metrics\n")
...
def main() -> None:
    ...
    final_model_path = paths["models_dir"] / "nasnetmobile_weapon_validator_final.keras"
    history_csv_path = paths["reports_dir"] / "nasnetmobile_training_history.csv"
    metrics_txt_path = paths["reports_dir"] / "nasnetmobile_test_metrics.txt"

    model.save(final_model_path)
    save_history(history, history_csv_path)
    save_test_metrics(test_metrics, metrics_txt_path, class_names)
